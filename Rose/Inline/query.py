...
from Rose.mongo.feddb import fed_promote, get_fed_from_ownerid, get_fed_name
...
@app.on_callback_query(filters.regex("promote"))
...
async def clear_rules(bot, query):
    ...
    await query.message.edit_text(
        comman_text.format(
            (query.message.from_user.mention[query.message.from_user.id]),
            "Rules",
...
@app.on_callback_query(filters.regex("unapprovecb"))
...
@app.on_callback_query(filters.regex("unpinallcb"))
async def unpinallcb(bot, query):
    ...
    try:
        ...
        return await app.send_message(chat_id=LOG_GROUP_ID, text=e)
...
@app.on_callback_query(filters.regex("new_"))
async def new_(bot, query):
    ...
    if chk == 404:
        return await query.message.edit(
            f"{query.message.from_user.mention},Captcha already tunned on {query.message.chat.title}[{query.message.chat.id}]\n use <code>/remove</code> command  to turn off"
        )
    else:
        await query.message.edit(
            f"{query.message.from_user.mention}[{query.message.from_user.id}] turn on {type_} Captcha in {query.message.chat.title}."
        )


@app.on_callback_query(filters.regex("off_"))
async def off_(bot, query):
    ...
    if j:
        await query.message.edit(
            f"{query.message.from_user.mention}[{query.message.from_user.id}] turn off Captcha in {query.message.chat.title}"
        )
...
@app.on_callback_query(filters.regex("jv_"))
async def off_(bot, query):
    chat_id = query.data.rsplit("_")[1]
    user_id = query.data.split("_")[2]
    ...
    if query.from_user.id not in localdatabase:
        return await query.answer(
            "Start me pm & Try Again After Re-Join!", show_alert=True
        )
    ...
    if _number not in localdatabase[query.from_user.id]["answer"]:
        ...
        if n == 0:
            ms = await query.message.edit_caption(
                f"{query.from_user.mention}[{query.from_user.id}], failed to solve the captcha in {query.chat.id}!\nYou can try again after 1 minutes.",
                reply_markup=None,
            )
        ...
        if not localdatabase[query.from_user.id]["answer"]:
            ...
            if "{username}" in text:
                text = text.replace(
                    "{username}", (await app.get_users(user_id)).username
                )
            if "{first}" in text:
                text = text.replace(
                    "{first}", (await app.get_users(user_id)).first_name
                )
            if "{last}" in text:
...
@app.on_callback_query(filters.regex("wrong_"))
...
@app.on_callback_query(filters.regex("groupcb"))
...
@app.on_callback_query(filters.regex("connectcb_"))
...
@app.on_callback_query(filters.regex("connectcb"))
...
@app.on_callback_query(filters.regex("deletecb"))
...
@app.on_callback_query(filters.regex("backcb"))
...
async def unwarn(bot, query):
    ...
    text = f"~~{text}~~\n\n"
    text += f"{comman_un_text}".format(
        "Warn", query.from_user.mention[{query.from_user.id}]
    )
...
async def unban(bot, query):
    ...
    text = f"~~{text}~~\n\n"
    text += f"{comman_un_text}".format(
        "Ban", query.from_user.mention[{query.from_user.id}]
    )
    await query.message.edit(text)
...
async def unmute(bot, query):
    ...
    text = f"~~{text}~~\n\n"
    text += f"{comman_un_text}".format(
        "Mute", query.from_user.mention[{query.from_user.id}]
    )
    await query.message.edit(text)
