...
from Rose import LOG_GROUP_ID
from Rose import MONGO_URL as MONGO_DB_URI
from Rose import app
from Rose import app as pbot
from Rose.core.decorators.permissions import (
    current_chat_permissions,
    member_permissions,
)
from Rose.mongo.locksdb import b_off, b_on, is_b_on, remove_chat
...
incorrect_parameters = (
    "<b>Incorrect lock types send </b><code>/locktypes</code><b> to see all.</b>"
)
...
async def tg_lock(message, permissions: list, perm: str, lock: bool):
    ...
    try:
        ...
        return await message.reply_text(
            "<b>To unlock this, you have to unlock</b><code>messages</code><b>first.</b>"
        )


@pbot.on_message(
    filters.command("lock") & ~filters.private & ~filters.via_bot & admin_filter
)
    ...
    try:
        ...
        if len(message.command) != 2:
            return await message.reply(
                "<b>Send something.. ): what do you want to lock in this chat ?</b>"
            )
        ...
        if "can_restrict_members" not in permissions:
            return await message.reply_text(
                f"{message.from_user.mention},You need to be an admin with <b>restrict members<b> permission."
            )
        ...
        if parameter in permdata:
            ...
            return await message.reply_text(
                (
                    f"<b>Locked</b> {parameter}."
                    if state == "lock"
                    else f"<b>Unlocked</b> {parameter}."
                )
            )
            ...
            for i in range(0, (len(array4)) - 2):
                await tg_lock(
                    message, permissions, array4[i], True if state == "lock" else False
                )
...
async def list_locks_dfunc(_, message):
    ...
    try:
        ...
        supun = await app.send_message(
            chat_id=message.chat.id,
            text=f"Hey {message.from_user.mention} please start me pm now.",
        )
