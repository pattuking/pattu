...
from Rose.utils.caching import admin_cache_reload
    ...
    if message.reply_to_message:
        if len(message.command) > 1:
            ...
            return await app.send_message(
                chat_id, text=send, reply_to_message_id=reply_id
            )
        else:
...
@app.on_message(
    filters.command(["promote", "fullpromote", "midpromote"])
    & ~filters.edited
    & ~filters.private
    & promote_filter
)
async def promoteFunc(_, message: Message):
    ...
    if message.command[0][0] == "f":
        ...
        return await message.reply_text(
            f"{umention} <b>Was Fullpromoted By</b> {message.from_user.mention} <b>with</b><code>{title}</code><b>title</b>"
        )
    if message.command[0][0] == "m":
        ...
        await app.set_administrator_title(message.chat.id, user_id, title)
        return await message.reply_text(
            f"{umention} <b>Was Midpromoted By</b> {message.from_user.mention} <b>with</b><code>{title}</code><b>title</b>"
        )
    await app.promote_chat_member(
        chat_id=message.chat.id,
        user_id=user_id,
    ...
    await message.reply_text(
        f"{umention} <b>Was Promoted By</b> {message.from_user.mention} <b>with</b> <code>{title}</code> <b>title</b>"
    )


@app.on_message(
    filters.command("demote") & ~filters.edited & ~filters.private & promote_filter
)
...
async def set_group_username(client, message: Message, _):
    ...
    try:
        ...
        return await message.reply_text(
            "{} was set {} as new username of this chat".format(
