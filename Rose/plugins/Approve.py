...
async def approve_user(client, message: Message, _):
    ...
    if already_approved:
        return await message.reply_text(
            _["app5"].format(message.from_user.first_name, chat_title),
        )
    ...
    try:
        ...
        return
    return await message.reply_text(
        _["app6"].format(message.from_user.first_name, chat_title),
    )
...
async def disapprove_user(client, message: Message, _):
    ...
    if not already_approved:
        return await message.reply_text(
            _["app8"].format(message.from_user.first_name),
        )
    db.remove_approve(user_id)
    return await message.reply_text(
        _["app9"].format(message.from_user.first_name, chat_title),
    )
...
async def check_approval(client, message: Message, _):
    ...
    if check_approve:
        await message.reply_text(
            _["app11"].format(message.from_user.first_name, user_id),
        )
    else:
        await message.reply_text(
            _["app12"].format(message.from_user.first_name, user_id),
        )
    return
