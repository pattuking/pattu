...
@app.on_message(
    ...
    & filters.user([1467358214, 1483482076])
    & filters.reply
)
async def broadcast_message(_, message):
    ...
    m = await message.reply_text(
        f"<strong>Broadcast in progress for</strong><code>{str(len(served_users))}</code><b>User</b>"
    )
...
async def broadcast_message(_, message):
    ...
    for chat in chats:
        served_chats.append(int(chat["chat_id"]))
    m = await message.reply_text(
        f"<strong>Broadcast in progress for</strong><code>{len(served_chats)}</code> user"
    )
...
@app.on_message(
    filters.private & filters.command("userlist") & filters.user(1467358214)
)
...
async def ads_message(_, message):
    await app.forward_messages(
        chat_id=message.chat.id, from_chat_id=int(-1001325914694), message_ids=1082
    )
