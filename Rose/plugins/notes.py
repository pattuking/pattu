...
async def save_note(_, m: Message):
    ...
    if not userid:
        return await m.reply(
            f"You are anonymous admin. Use `/connect {m.chat.id}` in PM"
        )
    ...
    try:
        if msgtype == Types.TEXT:
            if button:
                try:
                    return await reply_text(
                        textt, reply_markup=button, disable_web_page_preview=True
                    )
                ...
                return await reply_text(
                    textt, quote=True, disable_web_page_preview=True
                )
        elif msgtype in (
            Types.STICKER,
            Types.VIDEO_NOTE,
            ...
            try:
                return await (await send_cmd(c, msgtype))(
                    m.chat.id,
                    getnotes["fileid"],
                    caption=textt,
                    reply_markup=button,
...
async def get_raw_note(c: app, m: Message, note: str):
    ...
    if msgtype == Types.TEXT:
        ...
        await (await send_cmd(c, msgtype))(
            m.chat.id, getnotes["fileid"], reply_to_message_id=msg_id
        )
    else:
        teks = getnotes["note_value"] or ""
        await (await send_cmd(c, msgtype))(
            m.chat.id, getnotes["fileid"], caption=teks, reply_to_message_id=msg_id
        )
    return
...
async def local_notes(_, m: Message):
    ...
    if curr_pref:
        pm_kb = ikb(
            [[("All Notes", f"https://t.me/szRosebot?start=notes_{grp_id}", "url")]]
        )
        return await m.reply_text(
            "Click on the button below to get notes!", reply_markup=pm_kb
        )
