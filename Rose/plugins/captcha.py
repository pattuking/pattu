                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            ...
                            InlineKeyboardButton(
                                text="Captcha Off ❌",
                                callback_data=f"off_{chat_id}_{user_id}",
                            ),
                        ],
                        [
                            InlineKeyboardButton(
                                text="Close Menu ✖️", callback_data=f"close_data"
                            )
                        ],
                    ]
                ),
        ...
        if (user_s.is_member is False) and (db.get(user_id, None) is not None):
            try:
                await app.delete_messages(
                    chat_id=chat_id, message_ids=db[user_id]["msg_id"]
                )
            except:
                pass
            return
        ...
        if db.get(user_id, None) is not None:
            try:
                await app.send_message(
                    ...
                    "He can try again after 5 minutes.",
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        text="🤖 Verify Now ",
                        callback_data=f"verify_{chat_id}_{user_id}",
                    ),
                    InlineKeyboardButton(
                        text="Pass ❗️", callback_data=f"_unmute_{user_id}"
                    ),
                ]
            ]
        ),
