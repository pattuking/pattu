...
async def add_connection(group_id, user_id):
    query = connectdb.find_one({"_id": user_id}, {"_id": 0, "active_group": 0})
...
async def active_connection(user_id):
    ...
    query = connectdb.find_one({"_id": user_id}, {"_id": 0, "group_details": 0})
    if not query:
        return None
...
async def make_inactive(user_id):
    update = connectdb.update_one({"_id": user_id}, {"$set": {"active_group": None}})
    return update.modified_count != 0
...
async def delete_connection(user_id, group_id):
    ...
    try:
        ...
        if update.modified_count == 0:
            return False
        query = connectdb.find_one({"_id": user_id}, {"_id": 0})
        if len(query["group_details"]) >= 1:
            ...
            connectdb.update_one({"_id": user_id}, {"$set": {"active_group": None}})
        return True
